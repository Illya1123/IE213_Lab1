...
const SubmittedContent = () => {
  ...
  const formattedEventType = eventType.join(' - ');
  return (
    <div className="SubmittedContent">
      ...
      <div className="content">
        ...
        <p>Loại tiệc: - {formattedEventType} / Địa điểm: {location}</p>
        </div>
        <div className="YC">
        <h4><i>Các yêu cầu kèm theo: </i></h4>
        <pre style={{ paddingLeft: '5%' }}>{formattedRequests}</pre>
        </div>
        <p style={{textAlign: "center"}}><i>Quý khách biết đến nhà hàng của chúng tôi qua: </i> {method.join(", ")}</p>
